for c =4:2:154
addpath ('/rri_disks/artemis/meltzer_lab/CANBIND/processed_RS2_after_epochdel/')
addpath ('/rri_disks/artemis/meltzer_lab/shared/toolboxes/eeglab13_6_5b/')
eeg_dir=dir('/rri_disks/artemis/meltzer_lab/CANBIND/processed_RS2_after_epochdel/')

addpath ('/rri_disks/artemis/meltzer_lab/CANBIND/New_post_Apr')
[~, txt] = xlsread('EEG_del_epoch.xlsx')    
size(txt)
...
EEG = pop_loadset('filename',eeg_dir(c).name,'filepath','/rri_disks/artemis/meltzer_lab/CANBIND/processed_RS2_after_epochdel/');
...
mean_absolute_theta = mean(all_data(:,4));
sd_absolute_theta = mean(all_data(:,4));
absolute_norm = (all_data(chan,4) - mean_absolute_theta)/sd_absolute_theta;

%relative calculations
relative_theta_max = max(all_data(:,6));
relative_theta_vector = (ones(76,1)*relative_theta_max);
mean_relative_theta = mean(all_data(:,6));
sd_relative_theta = std(all_data(:,6));
relative_norm = (all_data(chan,6) - mean_relative_theta)/sd_relative_theta;


%theta cordance
a_norm_half = absolute_norm - 0.5;
r_norm_half = relative_norm - 0.5;
cordance = absolute_norm + relative_norm;


all_data_ = [mean_absolute_theta,sd_absolute_theta,mean_relative_theta,sd_relative_theta,absolute_norm,relative_norm,cordance];
